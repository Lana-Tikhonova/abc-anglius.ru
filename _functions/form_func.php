	...
	foreach ($fields as $key=>$value) {
		if (is_array($value)) {
			...
			foreach ($param as $k=>$v) {
				...
				elseif ($v=='email')$data[$key] = isset($post[$key]) ? strtolower(filter_var($post[$key], FILTER_SANITIZE_EMAIL)) : '';
				elseif ($v=='boolean') {
	...
	foreach ($fields as $key=>$value) {
		if (is_array($value)) {
			...
			foreach ($param as $k=>$v) {
				...
				elseif ($v=='password') {
					if (strlen($post[$key])<8) $message[] = i18n('validate|not_valid_password');
				}
				//видимая капча
				elseif ($v=='captcha') {
