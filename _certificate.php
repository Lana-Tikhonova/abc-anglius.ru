				...
				elseif(!$ol['teacher']) {
					...
					elseif(isset($basket['results'][$number])) {
						...
						if(strtotime($order['date'])>strtotime('2016-12-13 00:00:00')) {
							...
							textinblock($img,$color2,$font3_path_i,94,$basket['results'][$number]['fio'],array('x'=>260,'y'=>1757,'w'=>1960,'h'=>0,'h_align'=>'center','v_align'=>'top'));
							textinblock($img,$color1,$font1_path,50,$ol['testname'].', '.$basket['workplace'],array('x'=>260,'y'=>2060,'w'=>1960,'h'=>0,'h_align'=>'center','v_align'=>'top'));
					...
					if($ct&&isset($order['place'])) {
						if(!$ct['teacher']) {
							...
							if(strtotime($order['date'])>strtotime('2016-12-13 00:00:00')) {
								...
								textinblock($img,$color1,$font3_path,44,$ct['name'],
								...
								//textinblock($img,$color2,$font3_path_i,94,$basket['fio2'],array('x'=>327,'y'=>1959,'w'=>1832,'h'=>0,'h_align'=>'center','v_align'=>'top'));
                                                                textinblock($img,$color2,$font3_path_i,70,$basket['fio2'],array('x'=>200,'y'=>1959,'w'=>2000,'h'=>0,'h_align'=>'center','v_align'=>'top'));
								textinblock($img,$color1,$font3_path,38,$basket['workplace'],array('x'=>327,'y'=>2150,'w'=>1832,'h'=>0,'h_align'=>'center','v_align'=>'top'));
								textinblock($img,$color1,$font3_path,38,'Координатор: '.$basket['fio'],array('x'=>327,'y'=>2434,'w'=>1832,'h'=>0,'h_align'=>'center','v_align'=>'top'));
								textinblock($img,$color1,$font_path,31,'регистрационный номер диплома '.$order['id'].sprintf('%02s',$number),array('x'=>327,'y'=>2529,'w'=>1832,'h'=>0,'h_align'=>'center','v_align'=>'top'));
								textinblock($img,$color1,$font_path,31,date2(($order['date_paid']!=0?$order['date_paid']:$order['date']),'d month y').' года           /           Anglius.ru',array('x'=>327,'y'=>3190,'w'=>1832,'h'=>0,'h_align'=>'center','v_align'=>'top'),false);
							} else {
								//старый
								$img=imagecreatefromjpeg('./certif_7HbghJk/2/2/'.$order['place'].'.jpg');
								$color1= imagecolorallocate($img, 72,92,139);
								$color2= imagecolorallocate($img, 60,60,60);
								switch($order['place']){
		                                                  case 1:$color3= imagecolorallocate($img, 178,157,51);break;
			...
			case 4:	if (!$order['place']) {$error++;}
					...
					if ($tt) {
						...
						if(strtotime($order['date'])>strtotime('2016-12-13 00:00:00')) {
							...
							textinblock($img,$color1,$font3_path,44,$tt['name'],
								array('x'=>327,'y'=>1360,'w'=>1832,'h'=>0,'h_align'=>'center','v_align'=>'top'));
							textinblock($img,$color1,$font3_path,44,$basket['name'],
								array('x'=>327,'y'=>1588,'w'=>1832,'h'=>0,'h_align'=>'center','v_align'=>'top'));
