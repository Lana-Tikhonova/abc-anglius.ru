...
$photos = $q['images'] ? unserialize($q['images']) : array();
$i=0;
if (is_array($photos)) foreach ($photos as $k=>$v) if (@$v['display']) {
	$i++;
	$size = getimagesize(ROOT_DIR.$path.'/'.$k.'/'.$v['file']);
	$images[$i] = array(
		'img'=>'/'.$path.'/'.$k.'/'.$v['file'],
		'height'=>$size[1],
		'preview'=>'/'.$path.'/'.$k.'/p-'.$v['file'],
	...
	<table>
		<tr>
			<?php
				foreach ($images as $k=>$v) {
				?>
			<td><a href="#" data-i="<?=$k?>" title="<?=htmlspecialchars($v['title'])?>"><img src="<?=$v['preview']?>" alt="<?=htmlspecialchars($v['title'])?>" /></a></td>
				<?php
				}
			?>
		</tr>
