...
<?php }
  if(($q['type']==1||$q['type']==8||in_array($q['type'], [91,92,93,94]))&&!$q['teacher']) {
//print_r($q);
    if($olympiad!=$q['parent']){
      if($olympiad!=0) echo '</div></div>';
      $olympiad=$q['parent'];
      echo '<div class="olymp">'.@$config['types'][$q['type']].' от '.date('Y.m.d',strtotime($q['date'])).' <a href="#" class="showol" data-i="'.$q['parent'].'">'.$q['name'].'</a><div id="ol'.$q['parent'].'" class="hid">';
    }
  }
  elseif($olympiad!=0){
    echo '</div></div>';
