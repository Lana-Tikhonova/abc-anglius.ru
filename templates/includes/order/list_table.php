<?php
if($q['type']==3){
    //$basket['name']=mysql_select('SELECT name FROM '.((isOnlineOlympiads($q['type'])) ? 'online_olympiads' : $config['types_modules'][$q['type']]).' WHERE id='.$q['parent'],'string');
    ...
    if($q['type']==1||$q['type']==8||isOnlineOlympiads($q['type'])) {
        $basket['name']=$q['name'];
    }
}
?>
