...
$form[] = array('checkbox','change','',array('name'=>'сменить email и пароль','attr'=>'onchange=$(this).closest(\'form\').find(\'input[name=email],input[name=password]\').prop(\'disabled\',!this.checked)'));
$form[] = array('checkbox','display',true,array('help'=>'если галка снята, то пользователь не сможет авторизироваться и восстановить пароль'));
...
function after_save() {
	...
	if ($get['id']==$user['id']) {
		//заменяем емейл и хеш в сессии если были изменены
		if(isset($post['email'])) {
			$_SESSION['user']['email'] = $post['email'];
			$_SESSION['user']['hash'] = $post['hash'];
		}
		user('re-auth');
	}
}