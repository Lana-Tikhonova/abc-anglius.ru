...
$delete['delete'] = "DELETE FROM `seo_links-pages` WHERE child = '".$get['id']."'";
...
if ($get['u']=='similar_search') {
	...
	else $where = " LOWER(name) LIKE '%".mysql_real_escape_string(mb_strtolower($search,'UTF-8'))."%' OR LOWER(url) LIKE '%".mysql_real_escape_string(mb_strtolower($search,'UTF-8'))."%' ";
...
if ($get['u']=='delete_pages') {
	...
	mysql_query("TRUNCATE seo_pages");
}
...
if (isset($get['search']) && $get['search']!='') $where.= "
	AND (
		LOWER(seo_pages.name) like '%".mysql_real_escape_string(mb_strtolower($get['search'],'UTF-8'))."%'
