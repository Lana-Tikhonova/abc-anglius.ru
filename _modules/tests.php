...
$list=mysql_select('select * from tests where display=1 order by rank desc','rows_id');

if(!$u[2]){
	//просто список
	$html['content'] = html_array('tests/normal',$list);
}
else {
...
//		LIMIT 1
//	",'row')) {
//		if (explode2('-',$u[2],2)!=$data['url']) die(header('location: /'.$modules['tests'].'/'.$data['id'].'-'.$data['url'].'/'));
//	}
        if(isset($list[$id])) {
		$data=$list[$id];
		if (!$u[3]&&explode2('-',$u[2],2)!=$data['url']) die(header('location: /'.$modules['tests'].'/'.$data['id'].'-'.$data['url'].'/'));
	}
	else $error++;
...
//						}
				...
				$html['content'] = html_array('tests/form',@$post);
			}
		}
		else {
			...
			$breadcrumb=array();
			$html['content'] = html_array('tests/current',$list);
		}
	}
}
