...
//		) $query=$query+$q1;
		if($q1 = mysql_select("SELECT o.*,ol.name,ol.img,ol.teacher
			...
			ORDER BY teacher ASC,parent ASC",'rows_id')
		) $query=$query+$q1;
		...
		if($statuses[$u[3]]==2) {
			$query = "
				SELECT *
				FROM orders
				WHERE user = '".$user['id']."' AND paid=1 AND type>=3 AND type<5
				ORDER BY date ASC
			";
			$data=array();
	...
	elseif(intval($u[3])) {
		...
		",'row')) {
			...
			$order['more']=mysql_select('SELECT * FROM '.$config['types_modules'][$order['type']].' WHERE id='.$order['parent'],'row');
			switch($u[4]){
				case '':
					break;
				case 'edit':
...
//					exit;
			...
			$breadcrumb['module'][0] = array(0=>'Заявка №'.$u[3],1=>$config['profile'][$u[2]],'/'.$modules['profile'].'/'.$u[2].'/'.$u[3].'/');
	                if(file_exists(ROOT_DIR.'templates/includes/order/'.$order['type'].'.php'))
