...
if (count($_POST)>0) {
	...
	if (count($message)==0) {
		$result = mysql_query("
			...
			LIMIT 1
		");
                $q = (mysql_num_rows($result)==1) ? mysql_fetch_array($result) : array();                
		if ($q && $q['display']) {
			...
			*/
                ...
                elseif ($q && !$q['display']) 
                {
                    $message[] =i18n('validate|user_banned');
                }
		else 
                    $message[] =i18n('validate|no_email',true);//'Данный E-mail не закреплён ни за одним пользователем!';
	}
