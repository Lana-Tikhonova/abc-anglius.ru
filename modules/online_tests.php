...
//				пишем заказ и редиректим на оплату
				...
				$p['basket']=array(
					...
					'io'=>$userfields[1][0],
					'kk'=>'',
					'template'=> 1, //!empty($_POST['template']) ? $_POST['template'] : 1,
				);
				...
				if    ($percent>=floatval(str_replace(',','.',$config['percent_online1']))) $p['place']=1;
				elseif($percent>=floatval(str_replace(',','.',$config['percent_online2']))) $p['place']=2;
				elseif($percent>=floatval(str_replace(',','.',$config['percent_online3']))) $p['place']=3;
		...
		elseif(strval($u[3]+0)!=$u[3]||intval($u[3])<1||count($q['qa'])<intval($u[3])) {
			...
			$breadcrumb['page']=array(array($q['olname'],'/'.$q['olurl'].'/'),array($config['online_types'][$q['type']],'/'.$modules['online_olympiads'.$q['type']].'/'));
